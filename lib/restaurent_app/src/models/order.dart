...
//    map["driver_id"] = orderStatus?.id;
  ...
  Map acceptMap() {
    var map = new Map<String, dynamic>();
    map["id"] = id;
    map["orderStatus"]= OrderStatus.fromJSON({'id':1,'status':'Order Received'});
    if (orderStatus?.id != null && orderStatus?.id == '1') map["active"] = true;
    return map;
  }

  bool canCancelOrder() {
    return this.active == true && this.orderStatus.id == '1'||this.orderStatus.id == '6'; // 1 for order received status
  }
