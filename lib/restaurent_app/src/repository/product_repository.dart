...
Future<Stream<Product>> getProducts() async {
  Uri uri = Helper.getUri('api/product/all');
...
Future addProduct( Product product) async {
  String url = '${GlobalConfiguration().getValue('api_base_url')}products';
  final client = new http.Client();
  var user = userRepo.currentUser.value;
  url = url.replaceFirst("/manager", "");

  print('making request with url $url');
  print(product.toJson(user.apiToken));

  try {
    final response = await client.post(
      url,
      headers: {HttpHeaders.contentTypeHeader: 'application/json'},
      body: json.encode(product.toJson(user.apiToken)),
    );
    if (response.statusCode == 200) {
