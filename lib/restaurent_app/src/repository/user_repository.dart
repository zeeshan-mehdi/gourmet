...
Future<User> login(User user) async {
  final String url = '${GlobalConfiguration().getValue('api_base_url')}manager/login';
...
Future<bool> resetPassword(User user) async {
  final String url = '${GlobalConfiguration().getValue('api_base_url')}manager/send_reset_link_email';
  final client = new http.Client();
  final response = await client.post(
    url,
    headers: {HttpHeaders.contentTypeHeader: 'application/json'},
...
Future<User> updateCurrentUser() async {
  print('printing out current user');
  final String _apiToken = 'api_token=${currentUser.value.apiToken}';

  print('user id ${currentUser.value.id}');


  final String url = '${GlobalConfiguration().getValue('api_base_url')}manager/user?user_id=${currentUser.value.id}&$_apiToken';
...
Future<User> update(User user) async {
  final String _apiToken = 'api_token=${currentUser.value.apiToken}';
  final String url = '${GlobalConfiguration().getValue('api_base_url')}manager/users/${currentUser.value.id}?$_apiToken';
  final client = new http.Client();
  final response = await client.post(
    url,
    headers: {HttpHeaders.contentTypeHeader: 'application/json'},
...
Future<Address> addAddress(Address address) async {
  ...
  final String url = '${GlobalConfiguration().getValue('api_base_url')}manager/delivery_addresses?$_apiToken';
  final client = new http.Client();
  try {
    final response = await client.post(
      url,
...
Future<Address> updateAddress(Address address) async {
  ...
  final String _apiToken = 'api_token=${_user.apiToken}';
  address.userId = _user.id;
  final String url = '${GlobalConfiguration().getValue('api_base_url')}manager/delivery_addresses/${address.id}?$_apiToken';
  final client = new http.Client();
  try {
