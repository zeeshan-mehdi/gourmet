...
class _MobileVerificationBottomSheetWidgetState
  ...
  void _showToast(BuildContext context) {
    ...
    scaffold.showSnackBar(
      SnackBar(
        content: Text('send sms to ${widget.user.phone}'),
        action: SnackBarAction(label: 'UNDO', onPressed: scaffold.hideCurrentSnackBar),
      ),
    );
  }
  verifyPhone() async {
    final string = 'sending sms to ${widget.user.phone}';
    print(string);
   // ScaffoldMessenger.of(scaffoldKey?.currentContext).showSnackBar(SnackBar(
    //   content: Text("sending sms to ${widget.user.phone})"),
    // ));
    setState(() {
      isLoading = true;
    });
        ...
        child: Stack(
          children: <Widget>[
            ...
            Padding(
              ...
              child: ListView(
                padding:
                EdgeInsets.only(top: 0, bottom: 15, left: 8, right: 8),
                children: <Widget>[
                  Column(
                    children: <Widget>[
